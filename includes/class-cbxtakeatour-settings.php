...
if ( ! class_exists( 'CBXTakeaTour_Settings' ) ):
	class CBXTakeaTour_Settings {
		...
		function callback_heading( $args ) {
			$plus_svg  = cbxtakeatour_load_svg( 'icon_plus' );
			$minus_svg = cbxtakeatour_load_svg( 'icon_minus' );
			...
			$html = '<h3 class="setting_heading"><span class="setting_heading_title">' . esc_html( $args['name'] ) . '</span><a title="' . esc_attr__( 'Click to show hide',
					'cbxtakeatour' ) . '" class="setting_heading_toggle button outline primary icon-only icon-inline" href="#"><i class="cbx-icon cbx-icon-img setting_heading_toggle_plus">' . $plus_svg . '</i><i class="cbx-icon cbx-icon-img setting_heading_toggle_minus">' . $minus_svg . '</i></a></h3>';
